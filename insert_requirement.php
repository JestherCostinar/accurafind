...
if (isset($_POST['submitRequirement'])) {
    foreach ($_POST['file_desc'] as $key => $value) {
        ...
        $extensions_arr = array("jpg", "jpeg", "png", "doc", "docx", "pdx");
        // Check extension
        if (in_array($imageFileType, $extensions_arr)) {
            if (move_uploaded_file($_FILES["myfile"]["tmp_name"][$key], 'requirements-folder/' . $attachFile)) {
                $insertRequirement = mysqli_query($conn, "INSERT INTO tbljob_requirements(FILE_DESCRIPTION , FILE_NAME, APPLICANTID) VALUES ('$fileDesc', '$attachFile', '$applicantid')");
            } else {
                echo "<script>alert('Failed to upload file.')</script>";
            }
        } else {
            echo "<script>alert('You file extension must be .jpg, .jpeg .png .docs .pdf')</script>";
        }
    }
    if ($insertRequirement) {
        header("Location: applicant_profile.php?validateMsg=Requirements has been uploaded.");
        exit();
    } else {
    ...
    $logs = mysqli_query($conn, "INSERT INTO tbl_log (NAME, LOG_TIME, ROLE, ACTIVITY) VALUE ('{$_SESSION['fullname']}', '$today', 'Applicant', 'Insert job requirements')");
                                            ...
                                            <div class="job-post-details p-4" style="background: #fff; border: 1px solid #ededed;">
                                                <div class="post-details1 mb-50">
                                                    ...
                                                    <form action="" method="POST" enctype='multipart/form-data'>
                                                            ...
                                                            <div class="card">
                                                                ...
                                                                <div class="card-body">
                                                                    <div class="educ-main-form">
                                                                        <div class="form-group">
                                                                            <label for="schoolName">File Description</label>
                                                        ...
                                                        <button name="submitRequirement" id="submit" type="submit" class="btn btn-primary">Submit</button>
